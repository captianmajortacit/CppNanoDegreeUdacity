...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    /*Copying the end node values to a local Node*/
    const float loc_hVal=node->distance(*end_node);
    return loc_hVal;
}
...
// - Return the pointer.
bool Compare(RouteModel::Node *arg_A, RouteModel::Node *arg_B)
{
    const float loc_f1 = arg_A->g_value + arg_A->h_value;
    const float loc_f2 = arg_B->g_value + arg_B->h_value;
    return loc_f1>loc_f2;
}
RouteModel::Node *RoutePlanner::NextNode() {
    sort(RoutePlanner::open_list.begin(), RoutePlanner::open_list.end(), Compare);
    RouteModel::Node *loc_lowestSum = open_list.back();
    open_list.pop_back();
    return loc_lowestSum;
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    auto loc_Node = current_node;
    while(loc_Node !=  start_node)
    {
        path_found.push_back(*loc_Node);
        distance += loc_Node->distance(*loc_Node->parent); 
