...
ChatLogic::ChatLogic()
{
    ...
    //// functionality shall be performed by rule of 5   
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
        ...
        while (getline(file, lineStr))
        {
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    {
                        //// STUDENT CODE
                        //// Using unique pointer instead of raw pointers
    ...
    ChatBot loc_obj("../images/chatbot.png");
    loc_obj.SetChatLogicHandle(this);
    // add chatbot to graph root node
    //_chatBot->SetRootNode(rootNode);
    loc_obj.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(loc_obj));
